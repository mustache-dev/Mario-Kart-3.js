...
import { getDriftLevel, SMOKE_VFX_SETTINGS } from "../constants.js";
...
const tempWheelPos = new Vector3();
const tempFlamePos1 = new Vector3();
const tempFlamePos2 = new Vector3();
...
export function Kart({
  ...
  function rotateWheels(left, right, delta) {
    ...
    yRotation.current = damp(yRotation.current, inputTurn.current, 4, delta);
  ...
  function getWheelPositions() {
    const wheelPositions = [wheel0, wheel1, wheel2, wheel3].map((wheel, index) => {
      wheel.current.getWorldPosition(tempWheelPos);
  ...
  useFrame((_, delta) => {
    if (wheel0.current && wheel1.current && wheel2.current && wheel3.current) {
      if (groundMeshes.current.length < 2) {
        scene.traverse((obj) => {
          if (obj.isMesh && obj.name.includes("ground") && obj.geometry.boundsTree) {
      ...
      flamePositionLeftRef.current.getWorldPosition(tempFlamePos1);
      flamePositionRightRef.current.getWorldPosition(tempFlamePos2);
      setFlamePositions([tempFlamePos1.clone(), tempFlamePos2.clone()]);
        ...
        <group position-y={-0.5} scale={1} rotation-y={Math.PI}>
          ...
          >
            ...
            <mesh
              ...
              rotation={[0.279, 0, 0]}
            />
            <group position={[0.5, 0.65, -1.5]} rotation-x={-Math.PI / 9} ref={flamePositionLeftRef}>
              <VFXEmitter ref={smoke1Ref} emitter="smoke" settings={SMOKE_VFX_SETTINGS} />
            </group>
            <group position={[-0.5, 0.65, -1.5]} rotation-x={-Math.PI / 9} ref={flamePositionRightRef}>
